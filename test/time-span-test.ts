...
describe("Constructor and set are using tryParse", () => {
	it("so just do some simple tests.", () => {
...
describe("TimeSpan instantiation", () => {
	it("from miliseconds, seconds... etc", () => {
...
describe("Check TimeSpan parts get/set:", () => {
	it("milliseconds", () => {
		var ts: TimeSpan;
		...
		ts = new TimeSpan("00:10.123"); ts.milliseconds = 10; expect(ts.totalMilliseconds).to.be.equal(10 * second + 10);
		ts = new TimeSpan("00:10.123"); ts.milliseconds = 987; expect(ts.totalMilliseconds).to.be.equal(10 * second + 987);
		ts = new TimeSpan("-00:10.123"); ts.milliseconds = 10; expect(ts.totalMilliseconds).to.be.equal(-(10 * second + 10));
		ts = new TimeSpan("-00:10.123"); ts.milliseconds = 987; expect(ts.totalMilliseconds).to.be.equal(-(10 * second + 987));
		expect(() => ts.milliseconds = -1).to.throw(Error);
		expect(() => ts.milliseconds = 1000).to.throw(Error);
		expect(() => ts.milliseconds = 33.01).to.throw(Error);
	});
	it("seconds", () => {
		var ts: TimeSpan;
		...
		expect(new TimeSpan(-1234.34).seconds).to.be.equal(1);
		ts = new TimeSpan("00:6.123"); ts.seconds = 10; expect(ts.totalMilliseconds).to.be.equal(10 * second + 123);
		ts = new TimeSpan("00:6.123"); ts.seconds = 5; expect(ts.totalMilliseconds).to.be.equal(5 * second + 123);
		ts = new TimeSpan("-00:6.123"); ts.seconds = 10; expect(ts.totalMilliseconds).to.be.equal(-(10 * second + 123));
		ts = new TimeSpan("-00:6.123"); ts.seconds = 5; expect(ts.totalMilliseconds).to.be.equal(-(5 * second + 123));
		expect(() => ts.seconds = -1).to.throw(Error);
		expect(() => ts.seconds = 60).to.throw(Error);
		expect(() => ts.seconds = 1.23).to.throw(Error);
	});
	it("minutes", () => {
		var ts: TimeSpan;
		...
		expect(new TimeSpan(-61234.34).minutes).to.be.equal(1);
		ts = new TimeSpan("6:6.123"); ts.minutes = 10; expect(ts.totalMilliseconds).to.be.equal(10 * minute + 6 * second + 123);
		ts = new TimeSpan("6:6.123"); ts.minutes = 5; expect(ts.totalMilliseconds).to.be.equal(5 * minute + 6 * second + 123);
		ts = new TimeSpan("-6:6.123"); ts.minutes = 10; expect(ts.totalMilliseconds).to.be.equal(-(10 * minute + 6 * second + 123));
		ts = new TimeSpan("-6:6.123"); ts.minutes = 5; expect(ts.totalMilliseconds).to.be.equal(-(5 * minute + 6 * second + 123));
		expect(() => ts.minutes = -1).to.throw(Error);
		expect(() => ts.minutes = 60).to.throw(Error);
		expect(() => ts.minutes = 1.23).to.throw(Error);
	});
	it("hours", () => {
		var ts: TimeSpan;
		...
		ts = new TimeSpan("6:10:10.123"); ts.hours = 10; expect(ts.totalMilliseconds).to.be.equal(10 * hour + 10 * minute + 10 * second + 123);
		ts = new TimeSpan("6:10:10.123"); ts.hours = 5; expect(ts.totalMilliseconds).to.be.equal(5 * hour + 10 * minute + 10 * second + 123);
		ts = new TimeSpan("-6:10:10.123"); ts.hours = 10; expect(ts.totalMilliseconds).to.be.equal(-(10 * hour + 10 * minute + 10 * second + 123));
		ts = new TimeSpan("-6:10:10.123"); ts.hours = 5; expect(ts.totalMilliseconds).to.be.equal(-(5 * hour + 10 * minute + 10 * second + 123));

		expect(() => ts.hours = -1).to.throw(Error);
		expect(() => ts.hours = 24).to.throw(Error);
		expect(() => ts.hours = 1.34).to.throw(Error);
	});
	it("days", () => {
		var ts: TimeSpan;
		...
		ts = new TimeSpan("6:10:10.123"); ts.days = 10; expect(ts.totalMilliseconds).to.be.equal(10 * day + 6 * hour + 10 * minute + 10 * second + 123);
		ts = new TimeSpan("6:10:10.123"); ts.days = 5; expect(ts.totalMilliseconds).to.be.equal(5 * day + 6 * hour + 10 * minute + 10 * second + 123);
		ts = new TimeSpan("-6:10:10.123"); ts.days = 10; expect(ts.totalMilliseconds).to.be.equal(-(10 * day + 6 * hour + 10 * minute + 10 * second + 123));
		ts = new TimeSpan("-6:10:10.123"); ts.days = 5; expect(ts.totalMilliseconds).to.be.equal(-(5 * day + 6 * hour + 10 * minute + 10 * second + 123));

		expect(() => ts.days = -1).to.throw(Error);
		expect(() => ts.hours = 1.34).to.throw(Error);
	});
});


describe("tryParse specification", () => {
	...
	it("with timespan parameter should return new instance.", () => {
	...
	it("with valid total milliseconds as string value.", () => {
	...
	it("with invalid total milliseconds as string value.", () => {
	...
	it("with valid string value for pattern [+-][days.]hh:mm:ss[.milliseconds].", () => {
	...
	it("with invalid string value for pattern [+-][days.]hh:mm:ss[.milliseconds].", () => {
...
describe("TimeSpan operations", () => {
	it("like addition, substraction etc...", () => {
...
describe("TimeSpan formatting", () => {
	...
	it("invalide pattern.", () => {
		expect(() => TimeSpan.from(1, 1, 1, 1, 1, false).toString("%")).to.throw(Error);
		expect(() => TimeSpan.from(1, 1, 1, 1, 1, false).toString("%t%")).to.throw(Error);
		expect(() => TimeSpan.from(1, 1, 1, 1, 1, false).toString("%%%")).to.throw(Error);
	});
});
